  ...
  return (
    <div className="w-full space-y-6 pb-24 md:pb-12">
      ...
      <div className="flex items-center justify-between bg-base-100 p-4 md:p-0 rounded-2xl md:bg-transparent shadow-sm md:shadow-none sticky top-0 md:relative z-20 mx-4 md:mx-0">
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div className="min-w-0">
            <h2 className="text-lg md:text-2xl font-black text-gray-900 truncate">Editar Paciente</h2>
            <p className="text-[10px] md:text-sm font-bold text-gray-400 uppercase tracking-widest truncate">{patient.first_name} {patient.last_name}</p>
          </div>
        </div>
      ...
      <form action={async (fd: FormData) => {
        'use server'
        await updatePatient(slug, id, fd)
      }} className="space-y-6">
        ...
        <div className="hidden md:flex gap-4 px-4 md:px-0">
          ...
          <Link
            href={`/${slug}/admission/patients/${id}`}
            className="btn btn-ghost border-gray-200 rounded-2xl px-8 h-14 font-black"
          >
            CANCELAR
          </Link>
        </div>
